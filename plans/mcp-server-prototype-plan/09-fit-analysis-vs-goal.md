...
**Your goal (restated):** an MCP server that replicates the _minimum
functionality of the VS Code extension_ and communicates with the _browser
extension_ that drives chatbot websites.
...
that goal. It makes the MCP server a **client** that _depends on the VS Code
extension still running_, instead of a **replacement** for it. One architectural
...
| ---------------------------- | ------------------------------------------ | --------------------- | ------------------------- |
| **WebSocket server** (relay) | the **VS Code editor extension** spawns it | —                     | _hosts_ `localhost:55155` |
...
5. (Today) the response _text_ comes via the OS clipboard, not the message.
...
> Step 2: _"connects to the existing CodeWebChat WebSocket server as a VS Code-role
> client … the fastest prototype path because CodeWebChat already supports
> multiple editor-role clients."_
...
- Pros: this _is_ the replacement; works headless; the browser extension needs
...
| 5 — tests (mocked WS)                               | unit/integration                                                                                                    | ✅ Mocking the WS boundary works for both modes. In host mode you'll also drive a fake _browser_ client.                                                                                              | Keep; add host-mode tests.                                                          |
| 6 — protocol upgrade (v1)                           | `request_id`, response text in payload                                                                              | ✅✅ Especially valuable in host mode — **you own the server**, so you can add `request_id` and inline response text _without the clipboard_ on your side. This removes your most fragile dependency. | Prioritise once Mode B exists.                                                      |
...
inline payload) and prefer it over the clipboard — keeping the clipboard path as a
retained fallback (decision: do NOT remove it). This turns your single biggest
reliability risk into a backup rather than the only path. (This may need a small
browser-extension change; check `apps/browser` since it's in this same repo.)
