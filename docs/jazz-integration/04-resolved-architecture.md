...
No port 55155, no `gemini-coder` token; the reply can ride inline in
`chat_responses.response_text`, **with OS clipboard retained as a fallback capture
path** (do NOT remove clipboard support).
...
3. **Extension side:** `createDb({ driver:'memory', serverUrl, secret })` in the
   ...
   `chat_responses` with the reply text (inline option **in addition to** the
   retained clipboard copy — do not remove it). Feature-flag it against the current
   WS client.
...
- **Adds an inline reply path** (the reply can ride in the row) **alongside the
  retained clipboard** — reframes "Phase C" as additive; clipboard is NOT removed.
